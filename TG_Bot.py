...
class Users:
    ...
    def update_location(self, update: Update) -> None:
        """remove the pin message then pin the message which is storing the location data"""
        self.chatroom.unpin_all_messages()
        self.chatroom.pin_message(update.message.message_id)
        pass

    def visualize_location(self) -> None:
        """take the location data with latitude and longitude then present it to user"""
...
class TG_Clients:
    ...
    def enter_conversation(update: Update, context: CallbackContext):
        """when user send the location data, the conversation will be engaged """
        ...
        update.message.reply_location(                          # copy the location message from user then send to it again with inline keyboard
    ...
    def _hear_response(update: Update, context: CallbackContext):
        """wait for the user clicking the inline keyboard"""
    ...
    def _pin_location_message(update: Update):
        """through pinning location message, teelgram will save the location as a target location"""
    ...
    def _point2point_matching(update: Update):
        """
        getting the location data from pinned message and current location data from telegram 
        then print the avaliable bus options in the inline keyboard
        finally, back to the hear_response to wait for user's response
        """
        chatroom = BOT.get_chat(update.callback_query.message.chat.id)
    ...
    def _stop_search(update: Update):
        """
        receive the user location data then perform the stop search function in Bus.py
        then print the avaliable bus options in the inline keyboard
        finally, back to the hear_response to wait for user's response
        """
        chatroom = BOT.get_chat(update.callback_query.message.chat.id)
    ...
    def _show_search_info(update: Update):
        """to present the information of bus route"""
...
"""
    ...
    def _show_match_info(update: Update):
        """present the information of matched bus route"""
